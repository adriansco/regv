  ...
  $(function () {
    var Selector = ".dropdown-toggle[data-play]",
      parent =
        $(
          Selector
        ).parent(); /* From BS-Doc: All dropdown events are fired at the .dropdown-menu's parent element. */
    ...
    parent.on("show.bs.dropdown", function (e) {
      ...
      if (!target || !target.length) $.error("No target for play-animation");
      else if ($.fn.animo && animation)
...
(function ($, window, document) {
  ...
  $(function () {
    ...
    (function () {
      ...
      $(Selector).each(function () {
        var source =
          $(this).data("source") || $.error("Bar: No source defined.");
    ...
    (function () {
      ...
      $(Selector).each(function () {
        var source =
          $(this).data("source") || $.error("Bar Stacked: No source defined.");
        var chart = new FlotChart(this, source),
          option = {
            series: {
              stack: true,
    ...
    (function () {
      ...
      $(Selector).each(function () {
        var source =
          $(this).data("source") || $.error("Area: No source defined.");
        var chart = new FlotChart(this, source),
          option = {
            series: {
              lines: {
    ...
    (function () {
      ...
      $(Selector).each(function () {
        var source =
          $(this).data("source") || $.error("Line: No source defined.");
        var chart = new FlotChart(this, source),
          option = {
            series: {
              lines: {
    ...
    (function () {
      ...
      $(Selector).each(function () {
        var source =
          $(this).data("source") || $.error("Pie: No source defined.");
        var chart = new FlotChart(this, source),
          option = {
            series: {
              pie: {
    ...
    (function () {
      ...
      $(Selector).each(function () {
        var source =
          $(this).data("source") || $.error("Donut: No source defined.");
        var chart = new FlotChart(this, source),
          option = {
            series: {
              pie: {
...
(function ($, window, document) {
  ...
  $.extend(Markdownarea.prototype, {
          ...
          var render = function () {
            ...
            $this.marked($this.currentvalue, function (err, markdown) {
              ...
              $this.element
                ...
                .trigger("markdownarea-update", [$this]);
            });
          };
          render();
        ...
        codeScroll = this.code.find(".CodeMirror-scroll").on(
          ...
          $.Utils.debounce(function () {
            ...
            var codeHeight = codeContent.height() - codeScroll.height(),
              previewHeight =
                previewContainer[0].scrollHeight - previewContainer.height(),
      ...
      this.markdownarea.on(
        ...
        function (e) {
          ...
          if ($this.markdownarea.attr("data-mode") == "tab") {
            $this.markdownarea
              .find(".uk-markdown-button-markdown, .uk-markdown-button-preview")
              .removeClass("uk-active")
              .filter(this)
              .addClass("uk-active");

            $this.activetab = $(this).hasClass("uk-markdown-button-markdown")
              ? "code"
              : "preview";
    ...
    _buildtoolbar: function () {
      ...
      this.options.toolbar.forEach(function (cmd) {
        if (Markdownarea.commands[cmd]) {
          var title = Markdownarea.commands[cmd].title
            ? Markdownarea.commands[cmd].title
            : cmd;
          ...
          if (Markdownarea.commands[cmd].shortcut) {
            $this.registerShortcut(
              Markdownarea.commands[cmd].shortcut,
              Markdownarea.commands[cmd].action
            );
          }
        }
      });
      ...
      this.markdownarea.on("click", "a[data-markdownarea-cmd]", function () {
        ...
        if (
          cmd &&
          Markdownarea.commands[cmd] &&
          (!$this.activetab || $this.activetab == "code" || cmd == "fullscreen")
        ) {
    ...
    getMode: function () {
      ...
      return this.editor.getTokenAt(pos).state.base.htmlState
        ? "html"
        : "markdown";
    },
  });
  ...
  Markdownarea.commands = {
    fullscreen: {
      ...
      action: function (editor) {
        editor.markdownarea.markdownarea.toggleClass(
        ...
        if (
          editor.markdownarea.markdownarea.hasClass(
            "uk-markdownarea-fullscreen"
          )
        ) {
        ...
        editor.refresh();
        editor.markdownarea.preview
          .parent()
          .css("height", editor.markdownarea.code.height());
      },
    },
    ...
    bold: {
      ...
      action: function (editor) {
        baseReplacer(
          this.getMode() == "html" ? "<strong>$1</strong>" : "**$1**",
          editor
        );
      },
    },
  ...
  Markdownarea.template =
    '<div class="uk-markdownarea uk-clearfix" data-mode="split">' +
  ...
  $(function () {
    $("textarea[data-uk-markdownarea]").each(function () {
      ...
      if (!area.data("markdownarea")) {
        obj = new Markdownarea(
          area,
          $.Utils.options(area.attr("data-uk-markdownarea"))
        );
      }
    });
  });
...
(function ($, window, document) {
  $(function () {
    ...
    $(document)
      ...
      .on(
        "click",
        openSelector + ", " + inputSelector + ", " + dismissSelector,
        function (e) {
          e.stopPropagation();
        }
      )
  ...
  $(function () {
    $(document)
      .on("click", collapseSelector, function (e) {
        ...
        if ($(window).width() > phone_mq && $body.hasClass(toggledClass))
          return;
      ...
      .on("click", ".sidebar > .nav > li", function () {
        if ($body.hasClass(toggledClass) && $(window).width() > phone_mq) {
          $(".sidebar > .nav > li")
            .not(this)
            .removeClass("open")
            .end()
...
(function ($, window, document) {
  ...
  $(document).on("click", Selector, function (e) {
    ...
    if ($this.hasClass("open")) btn.dropdown("toggle");
  });
})(jQuery, window, document);
...
$(document).ready(function () {
  ...
  /* focusout */
  $("#mdp-demo").focusout(function () {
    var value = document.getElementById("mdp-demo").value;
    document.getElementById("mdp-demo2").value = value;
    console.log("hi");
    /* $('#mdp-demo2').multiDatesPicker('resetDates', 'picked'); */
  });
  $("#mdp-demo2").focusout(function () {
    var value = document.getElementById("mdp-demo2").value;
    document.getElementById("mdp-demo3").value = value;
  });
});

function test() {
  /* function workingDays(dateFrom, dateTo) { */
  var from = moment(/* dateFrom */ "2021/02/18", "YYY/MM/DD"),
    to = moment(/* dateTo */ "2021/02/22", "YYY/MM/DD"),
    days = 0;
...
function test2() {
  var dateFrom = "18/02/2021";
  var dateTo = "23/02/2021";
...
jQuery(function ($) {
  ...
  $("#salida, #regreso, #fplanta").datepicker({
    format: "yyyy/mm/dd",
    autoclose: true,
    language: "es",
  });
  ...
  $("#salida").on("change", function () {
    var fecha = $("#salida").datepicker("getDate");
    if (fecha === null) $("#regreso").datepicker("clearDates");
    else fecha.setDate(fecha.getDate() + 1);
    $("#regreso").datepicker("setStartDate", fecha);
  });
...
jQuery(function ($) {
  $("#empcontrol").on("change", function () {
    var empcontrol = document.getElementById("empcontrol").value;
    document.getElementById("passcontrol").value = empcontrol;
    document.getElementById("usercontrol").value = empcontrol;
    console.log(empcontrol);
  });
});
// Actualizar
$(document).on("submit", ".form_update", function (event) {
  ...
  if (data_form.email.length <= 0) {
    $("#msg_error").text("Usuario vacío.").show();
    return false;
  } else if (data_form.password.length <= 0) {
    $("#msg_error").text("Password vacio.").show();
    return false;
  }
  $("#msg_error").hide();
  var url_php = "?c=empleado&a=valData";
...
$(document).on("click", ".editbtn", function (e) {
  e.preventDefault();
  var id = $(this).data("id");
  var val = $(this).data("test");
  /* document.getElementById("ctrl-empleado").value = id; */
  $("#slt-dias").prop("disabled", true);
  /* Guardamos el select de dias */
  var dias = $("#slt-dias"),
  ...
  if (val != "") {
    $.ajax({
      data: { id: id },
      url: "?c=empleado&a=diasPorEmpleado",
      type: "POST",
      dataType: "json",
      beforeSend: function () {
      ...
      success: function (r) {
        document.getElementById("ctrl-empleado").value = r[0].nombre;
        /* Limpiamos el select */
        dias.find("option").remove();
        $(r).each(function (i, v) {
          // indice, valor
          dias.append('<option value="' + v.id + '">' + v.name + "</option>");
        });
    ...
    dias.find("option").remove();
    document.getElementById("ctrl-empleado").value = "";
    dias.append(
      "<option value=''>No tiene días adicionales para disfrutar</option>"
    );
...
$(".btn-register").click(function () {
  ...
  var dive = document.getElementById("div-entrada"),
    divc = document.getElementById("div-comm");
  if ($("#entrada").val().length == 0) {
    Swal.fire({
      title: "Oops...",
      text: "El campo fecha en que lo tomara no puede estar vacío.",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        dive.className += " has-error";
      } else {
        dive.className += " has-error";
      }
    });
    btn.innerText = "Aprobar día";
  } else if ($("#exampleFormControlTextarea1").val().length == 0) {
    Swal.fire({
      title: "Oops...",
      text: "El campo observaciones no puede estar vacío.",
      icon: "warning",
      showCancelButton: false,
      confirmButtonColor: "#4ea5e0",
      confirmButtonText: "OK",
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        var form = new FormData($("#myModal")[0]);
        form.append("view_type", "addtemplate");
        $.ajax({
          type: "POST",
          url: "?c=empleado&a=approveExtraDay",
          data: form,
          cache: false,
          contentType: false,
          processData: false,
          success: function (data) {
            /* console.log("---" + data); */
            Swal.fire("¡Guardado!", "Tus cambios se han guardado.", "success");
        ...
        /* $(".btn-register").text("Aprobar día"); */
        btn.innerText = "Aprobar día";
      }
    });
  }
  /* https://www.youtube.com/watch?v=oxWv74OI6xA */
  /* deshabilitar botón form */
  /* $("#frm-alumno")
    .closest("form")
